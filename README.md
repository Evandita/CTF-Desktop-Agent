...
**`ctf-agent run <TASK> [OPTIONS]`** -- Execute a single task non-interactively.

Same options as `interactive`. Runs the task once and exits.
...
# Run container standalone (for development)
make container

# Stop and remove
make container-stop

# Connect agent to existing container
ctf-agent interactive --no-container --api-url http://localhost:8888
```
...
ctf-desktop-agent/
  ...
  src/ctf_agent/
    agent/
      core.py                 # Agent loop with HITL interception
      tools.py                # Tool base class and registry
      prompts.py              # System prompt templates
      planner.py              # Task planning via LLM
    config/
      models.py               # Pydantic config models
      settings.py             # Config loading and env overrides
    container/
      manager.py              # Docker container lifecycle
      client.py               # Async HTTP client to container API
    hitl/
      manager.py              # HITL coordination (futures-based)
      bridge.py               # HTTP bridge for inter-process HITL
    interfaces/
      cli.py                  # Click CLI with rich output
      mcp_server.py           # MCP server for Claude Code
      web/
        app.py                # FastAPI web app + WebSocket
        static/
          index.html          # Web UI layout
          app.js              # Frontend logic + HITL dialogs
          style.css           # Styles
    llm/
      base.py                 # LLM provider interface
      claude_provider.py      # Anthropic API provider
      ollama_provider.py      # Ollama local provider
      claude_code_provider.py # Claude Code CLI provider
      factory.py              # Provider factory
      message_types.py        # Message/content dataclasses
